    ...
    <div ref={ref} className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          <span className="block text-sm font-medium text-black dark:text-white leading-tight">
            {fullName}
          </span>
          <span className="block text-xs text-gray-400 leading-tight mt-0.5">
            {SUB_ROLE_LABELS['SUPER_ADMIN'] ?? 'Admin'}
          </span>
        </span>

        <div className="relative h-10 w-10 flex-shrink-0 rounded-full bg-primary/15 flex items-center justify-center text-primary font-bold text-sm ring-2 ring-transparent group-hover:ring-primary/30 transition">
          {initials}
        </div>

        <svg
          className={`hidden fill-current sm:block text-gray-400 transition-transform ${open ? 'rotate-180' : ''}`}
          width="11"
          height="7"
          viewBox="0 0 12 8"
          fill="none"
      ...
      {open && (
        <div className="absolute right-0 mt-3 w-64 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark z-50">
          {/* User info header */}
          <div className="flex items-center gap-3 px-5 py-4 border-b border-stroke dark:border-strokedark">
            <div className="h-11 w-11 flex-shrink-0 rounded-full bg-primary/15 flex items-center justify-center text-primary font-bold">
              {initials}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-black dark:text-white truncate">{fullName}</p>
              <p className="text-xs text-gray-400 truncate">{user?.email}</p>
            </div>
          </div>

          {/* Menu items */}
          <ul className="flex flex-col py-2 border-b border-stroke dark:border-strokedark">
            <li>
              <Link
                to="/settings"
                onClick={() => setOpen(false)}
                className="flex items-center gap-3 px-5 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:text-primary hover:bg-gray-50 dark:hover:bg-meta-4 transition"
              >
                <svg className="fill-current opacity-60" width="17" height="17" viewBox="0 0 22 22">
            ...
            <li>
              <Link
                ...
                className="flex items-center gap-3 px-5 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:text-primary hover:bg-gray-50 dark:hover:bg-meta-4 transition"
              >
                <svg className="fill-current opacity-60" width="17" height="17" viewBox="0 0 24 24">
          ...
          <div className="py-2">
            <button
              ...
              className="flex w-full items-center gap-3 px-5 py-2.5 text-sm text-meta-1 hover:bg-red-50 dark:hover:bg-meta-4 transition"
            >
              <svg className="fill-current" width="17" height="17" viewBox="0 0 22 22">
